            ...
            foreach($perk in $slot)
            {
                ...
                if(![string]::IsNullOrEmpty($perkHash))
                {
                    ...
                    try {
                        ...
                        Write-Verbose "Couldn't find hash for perk `"$perk`" ($($perkHash)) for $($item.name) [$($item.hash)] w/ $tagFooter!"
                    }
                }
            }
        ...
        foreach($rec in $this.recs)
        {
            ...
            foreach($variant in $this.variants)
            {
                ...
                try {
                    ...
                    write-verbose "error printing variant $($variant.item.hash)"
                }
            }
			$index++
        }
    ...
    [void] ParseItem($line)
    {
        if($line -match ".*https://light.gg/db/items/([0-9]+)/.*")
        {
            ...
            if($this.weapon) {
                ...
                {
                    write-verbose "finishing weapon $($this.weapon.item.name) [$($this.weapon.item.hash)]"
